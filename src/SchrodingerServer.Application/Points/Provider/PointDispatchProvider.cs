...
{
    Task<bool> GetDispatchAsync(string prefix, string bizDate);
    Task SetDispatchAsync(string prefix,string bizDate,bool isDispatched);
    Task<int> GetDailyChangeHeightAsync(string prefix, string bizDate);
    Task SetDailyChangeHeightAsync(string prefix,string bizDate,int height);
...
public class PointDispatchProvider : IPointDispatchProvider,ISingletonDependency
{
    ...
    public async Task<bool> GetDispatchAsync(string prefix, string bizDate)
    {
        var id = GetId(prefix, bizDate);
    ...
    public async Task SetDispatchAsync(string prefix,string bizDate,bool isDispatched)
    {
        var id = GetId(prefix, bizDate);
    ...
    public async Task<int> GetDailyChangeHeightAsync(string prefix, string bizDate)
    {
        var id = GetId(prefix, bizDate);
        var isDispatched = await _distributedCache.GetAsync(id);
        if (isDispatched != null)
    ...
    public async Task SetDailyChangeHeightAsync(string prefix,string bizDate, int height)
    {
        var id = GetId(prefix, bizDate);
    ...
    private string GetId(string prefix, string bizDate)
    {
        return $"{prefix}-{bizDate}";
    }
